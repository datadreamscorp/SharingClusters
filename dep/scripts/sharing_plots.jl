...
# ╔═╡ bccc43d0-2ccd-4cb5-8674-b9667a217a33
...
# ╔═╡ a6223444-efe5-4dd3-893b-ac45eea5019f
begin
	u = 0.5
	B = 10
	C = 0.1
...
W_s075(kbar; u=u, B=B, C=C, β=1, s=0.75) = kbar ≥ 1 ? u*log( (1-s)*B + (1 - (1 - u)^(β*kbar))*s*u*β*B - β*kbar*C ) + (1-u)*log( 1 + (1 - (1 - u)^(β*kbar))*s*u*β*B - β*kbar*C ) : u*log(B)

# ╔═╡ 011320ee-9010-4dfb-b13e-ad92e5e40e26
W_s05(kbar; u=u, B=B, C=C, β=1, s=0.5) = kbar ≥ 1 ? ( 1 + (1 - (1 - u)^(β*kbar))*s*u*β*B - β*kbar*C > 0 ?  u*log( (1-s)*B + (1 - (1 - u)^(β*kbar))*s*u*β*B - β*kbar*C ) + (1-u)*log( 1 + (1 - (1 - u)^(β*kbar))*s*u*β*B - β*kbar*C ) : 0 ) : u*log(B)
...
begin
	...
	mean_vec, std_vec = mean_degree_fitness(u, 0.75, B, C, β)
	mean_vec2, std_vec2 = mean_degree_fitness(u, 0.5, B, C, β)
...
begin
	...
	k_plot_math = plot(x_axis, W_s075,
...
begin
	kbar_bound, plog, kbar_no_bound = maxshare_Du(B=5, C=0.05)
...
begin
	khat_b5, kstar_b5 = calculate_points_Sim(s, 5, 0.05, 1, w_0=0.1, max_kbar=35, reps=1000)
	khat_b15, kstar_b15 = calculate_points_Sim(s, 15, 0.15, 1, w_0=0.1, max_kbar=35, reps=1000)
	khat_b30, kstar_b30 = calculate_points_Sim(s, 30, 0.3, 1, w_0=0.1, max_kbar=35, reps=1000)
end
...
begin
	...
	zeros1 = zero_plots( 
		calculate_points_Du(B=5, C=0.05, s=s), 
		ylim=ylimit, 
		lab=false 
	)
	scatter!(
	...
	zeros2 = zero_plots( 
		calculate_points_Du(B=15, C=0.15, s=s),  
		lab=false, 
		leg=false,
		ylim=ylimit,
		yax=nothing
	...
	zeros3 = zero_plots( 
		calculate_points_Du(B=30, C=0.3, s=s), 
		lab=false, 
		leg=false,
		ylim=ylimit,
		yax=nothing
